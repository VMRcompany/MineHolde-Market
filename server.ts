...
export function cleanSeoDescription(desc?: string): string {
  if (!desc) {
    return 'Скачать оригинальный мод для Minecraft Java Edition бесплатно на MineHolde Market.';
  }
  ...
  const slice = cleaned.slice(0, 157);
...
export function injectSeoIntoHtml(html: string, product: any, productUrl: string): string {
  ...
  const title = `Скачать мод ${rawTitle.trim()} на Minecraft Java Edition — MineHolde Market`;
