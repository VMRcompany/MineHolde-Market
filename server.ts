...
// Yandex ID OAuth Backend Configuration for Firebase Auth / Vercel
...
async function handleModrinthSearch(req: Request, res: Response) {
  ...
  const facets = rawFacets !== undefined ? rawFacets : (query ? '' : JSON.stringify([['project_type:mod']]));
...
// ----------------------------------------------------
// Reusable Product Lookup Function
// Checks in-memory catalog, Modrinth, and CFWidget to ensure dynamic routing
// works for ALL items, never returning 404 for valid items.
// ----------------------------------------------------
export async function resolveProductById(rawId: string): Promise<{ found: boolean; source: string; product: any } | null> {
  ...
  // 1. Check in-memory MARKETPLACE_PRODUCTS
  const mpProduct = MARKETPLACE_PRODUCTS.find(
    (p) =>
      p.id === id ||
      p.id.toLowerCase() === lowerId ||
      p.uuid === id ||
      p.uuid.toLowerCase() === lowerId ||
      p.id.toLowerCase() === strippedLower ||
      p.uuid.toLowerCase() === strippedLower
  );

  if (mpProduct) {
    return { found: true, source: 'marketplace', product: mpProduct };
  }

  // 2. Modrinth REST API lookup (checks both original-case project_id and lowercase slug)
  const modrinthIds = Array.from(new Set([strippedId, strippedLower, id, lowerId])).filter(Boolean);
  for (const mid of modrinthIds) {
    try {
      ...
      if (mrRes.ok) {
        ...
        if (hit && (hit.id || hit.slug)) {
          // Try to enrich with latest version download URL if available
          let resolvedDownloadUrl = hit.downloadUrl || '';
          let resolvedFilename = hit.downloadFilename || '';
          if (!resolvedDownloadUrl && hit.latest_version) {
            try {
              const vRes = await fetch(`https://api.modrinth.com/v2/version/${encodeURIComponent(hit.latest_version)}`, {
                headers: {
                  Accept: 'application/json',
                  'User-Agent': 'MineHolde/1.0.0 (contact@mineholde.market)',
                },
              ...
              if (vRes.ok) {
                ...
                if (primary?.url) {
                  resolvedDownloadUrl = primary.url;
                  resolvedFilename = primary.filename || '';
                }
              }
            } catch {}
          }
          ...
          const enriched = {
            ...
            downloadUrl:
              resolvedDownloadUrl ||
              (hit.latest_version
                ? `https://cdn.modrinth.com/data/${hit.id}/versions/${hit.latest_version}/${hit.slug || hit.id}.jar`
                : ''),
            downloadFilename: resolvedFilename || `${hit.slug || hit.id}.jar`,
          };

          return { found: true, source: 'modrinth', product: enriched };
        }
      }
    } catch {}
  }

  // 3. Modrinth search fallback for exact title or slug match
  try {
    const sRes = await fetch(`https://api.modrinth.com/v2/search?query=${encodeURIComponent(strippedLower)}&limit=5`, {
      headers: {
        Accept: 'application/json',
        'User-Agent': 'MineHolde/1.0.0 (contact@mineholde.market)',
      },
    ...
    if (sRes.ok) {
      ...
      if (Array.isArray(sData.hits) && sData.hits.length > 0) {
        const exactMatch = sData.hits.find(
          (h: any) =>
            String(h.project_id) === strippedId ||
            String(h.project_id).toLowerCase() === strippedLower ||
            String(h.slug).toLowerCase() === strippedLower ||
            String(h.title).toLowerCase().replace(/[^a-z0-9_-]/g, '') === strippedLower
        );
        if (exactMatch) {
          return { found: true, source: 'modrinth', product: exactMatch };
        }
      }
    }
  } catch {}

  // 4. CFWidget REST API lookup
  const cfIds = Array.from(new Set([strippedId, strippedLower, id, lowerId])).filter(Boolean);
  for (const cid of cfIds) {
    try {
      const urls = [
        `https://api.cfwidget.com/minecraft/mc-mods/${encodeURIComponent(cid)}`,
        `https://api.cfwidget.com/${encodeURIComponent(cid)}`,
      ];
      for (const u of urls) {
        const cfRes = await fetch(u, {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'MineHolde/1.0.0 (contact@mineholde.market)',
          },
        ...
        if (cfRes.ok) {
          ...
          if (cfData && (cfData.id || cfData.title || cfData.name)) {
            return { found: true, source: 'cfwidget', product: cfData };
          }
        }
      }
    } catch {}
  ...
  // 5. Autonomous infinite fallback: synthetically generate rich product structure from ID/slug
  const formattedTitle = strippedId
    .replace(/[-_]+/g, ' ')
    .split(' ')
  ...
  return {
    ...
    product: {
      ...
      description: `Скачать модификацию ${formattedTitle} для Minecraft Java Edition на MineHolde Market. Оригинальные моды, текстуры и дополнения.`,
      icon_url: 'https://market.mineholde.pro/icons/icon-512x512.png',
    },
  };
}

// ----------------------------------------------------
// SEO Metadata Helpers for Search Robots and Social Crawlers
// ----------------------------------------------------
export function extractProductSeoImage(p: any): string {
  let img =
    p.thumbnailUrl ||
    p.icon_url ||
    p.bannerUrl ||
    (Array.isArray(p.screenshots) && p.screenshots.length > 0
      ? typeof p.screenshots[0] === 'string'
        ? p.screenshots[0]
        : p.screenshots[0]?.url
      : '') ||
  ...
  if (!img) {
    img = 'https://market.mineholde.pro/icons/icon-512x512.png';
...
export function cleanSeoDescription(desc?: string): string {
  if (!desc) {
...
export function injectSeoIntoHtml(html: string, product: any, productUrl: string, section: string = 'mods'): string {
  const rawTitle = product.title || product.name || 'Товар';
  const title = buildSeoTitle(section, rawTitle);

  const rawDesc = product.description || product.shortDescription || product.summary || product.body || getFallbackDescription(section, rawTitle);
  const description = cleanSeoDescription(rawDesc);

  const imageUrl = extractProductSeoImage(product);
  ...
  const replaceOrInsertMeta = (tagRegex: RegExp, newTag: string) => {
    if (tagRegex.test(modified)) {
      modified = modified.replace(tagRegex, newTag);
    } else {
      modified = modified.replace('</head>', `  ${newTag}\n  </head>`);
    }
  };

  replaceOrInsertMeta(/<meta\s+name=["']description["'][^>]*>/i, `<meta name="description" content="${escapeHtml(description)}" />`);
  replaceOrInsertMeta(/<meta\s+property=["']og:title["'][^>]*>/i, `<meta property="og:title" content="${escapeHtml(title)}" />`);
  replaceOrInsertMeta(/<meta\s+property=["']og:description["'][^>]*>/i, `<meta property="og:description" content="${escapeHtml(description)}" />`);
  replaceOrInsertMeta(/<meta\s+property=["']og:image["'][^>]*>/i, `<meta property="og:image" content="${escapeHtml(imageUrl)}" />`);
  replaceOrInsertMeta(/<meta\s+property=["']og:url["'][^>]*>/i, `<meta property="og:url" content="${escapeHtml(productUrl)}" />`);
  replaceOrInsertMeta(/<meta\s+name=["']twitter:title["'][^>]*>/i, `<meta name="twitter:title" content="${escapeHtml(title)}" />`);
  replaceOrInsertMeta(/<meta\s+name=["']twitter:description["'][^>]*>/i, `<meta name="twitter:description" content="${escapeHtml(description)}" />`);
  replaceOrInsertMeta(/<meta\s+name=["']twitter:image["'][^>]*>/i, `<meta name="twitter:image" content="${escapeHtml(imageUrl)}" />`);

  return modified;
}

// Universal Product Lookup REST API Endpoint
app.get('/api/product-lookup', async (req: Request, res: Response) => {
  const rawId = String(req.query.id || '').trim();
  if (!rawId) {
    return res.status(400).json({ found: false, error: 'id parameter is required' });
  }

  const result = await resolveProductById(rawId);
  if (result) {
    return res.json(result);
  }

  return res.status(404).json({ found: false, error: 'Product not found in REST API' });
});

// Dynamic Serverless SEO Page renderer
app.all('/api/product-page', productPageHandler as any);
...
app.get('/api/download', async (req: Request, res: Response) => {
  ...
  if (fileUrl) {
    try {
      ...
      if (!finalName) finalName = 'mod.jar';
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    // Dynamic SEO pre-rendering for Googlebot, search engines, and social media crawlers
    app.get(['/mods/:id', '/resourcepacks/:id', '/shaders/:id', '/modpacks/:id', '/plugins/:id', '/servers/:id', '/skins/:id'], async (req, res, next) => {
      try {
        const id = decodeURIComponent(req.params.id || '').trim();
        const section = req.path.split('/')[1] || 'mods';
        const hit = await resolveProductById(id);
        if (hit && hit.product) {
          ...
          const enriched = injectSeoIntoHtml(transformed, hit.product, `https://market.mineholde.pro${req.originalUrl}`, section);
          return res.status(200).set({ 'Content-Type': 'text/html; charset=utf-8' }).end(enriched);
        }
      } catch (err) {
        console.warn('Dev SEO pre-render notice:', err);
      }
      next();
    });
    ...
    // Dynamic SEO pre-rendering for Googlebot, search engines, and social media crawlers
    app.get(['/mods/:id', '/resourcepacks/:id', '/shaders/:id', '/modpacks/:id', '/plugins/:id', '/servers/:id', '/skins/:id'], async (req, res) => {
      try {
        const id = decodeURIComponent(req.params.id || '').trim();
        const section = req.path.split('/')[1] || 'mods';
        const hit = await resolveProductById(id);
        if (hit && hit.product) {
          ...
          const enriched = injectSeoIntoHtml(rawTemplate, hit.product, `https://market.mineholde.pro${req.originalUrl}`, section);
          return res.status(200).set({ 'Content-Type': 'text/html; charset=utf-8' }).end(enriched);
        }
      } catch (err) {
        console.warn('Prod SEO pre-render notice:', err);
      }
