...
export function injectSeoIntoHtml(html: string, product: any, productUrl: string): string {
  const rawTitle = product.title || product.name || 'Товар';
  const title = `Скачать мод ${rawTitle.trim()} для Minecraft Java Edition — MineHolde Market`;

  const rawDesc = product.description || product.shortDescription || product.summary || product.body || '';
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== 'production') {
    ...
    app.get(['/mods/:id', '/skins/:id'], async (req, res, next) => {
      try {
        ...
        if (hit && hit.product) {
          ...
          const enriched = injectSeoIntoHtml(transformed, hit.product, `https://market.mineholde.pro${req.originalUrl}`);
    ...
    // Dynamic SEO pre-rendering for Googlebot, search engines, and social media crawlers
    app.get(['/mods/:id', '/skins/:id'], async (req, res) => {
      try {
        const id = decodeURIComponent(req.params.id || '').trim();
        const hit = await resolveProductById(id);
        if (hit && hit.product) {
          const rawTemplate = await fs.promises.readFile(path.join(distPath, 'index.html'), 'utf-8');
          const enriched = injectSeoIntoHtml(rawTemplate, hit.product, `https://market.mineholde.pro${req.originalUrl}`);
          return res.status(200).set({ 'Content-Type': 'text/html; charset=utf-8' }).end(enriched);
        }
      } catch (err) {
