 ...
 * and restricting to 150-160 characters for optimal Google search snippets.
 */
export function cleanProductDescription(desc?: string): string {
  ...
  // Truncate at word boundary around 155 chars
 ...
 * "Скачать мод [Название мода] на Minecraft Java Edition — MineHolde Market"
 */
export function formatProductTitle(title?: string): string {
  if (!title) return DEFAULT_TITLE;
  return `Скачать мод ${title.trim()} на Minecraft Java Edition — MineHolde Market`;
}
