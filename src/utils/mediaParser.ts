...
export function extractMediaFromApiItem(raw: any): {
  ...
  // Direct imageGuid or UUID property on root item or nested displayProperties
  // Formats strictly using CDN template https://playable-thumbnails.minecraft.net/images/[IMAGE_GUID]
  const directImageGuid =
    raw.imageGuid ||
    raw.image_guid ||
    raw.displayProperties?.imageGuid ||
    raw.displayProperties?.thumbnail?.imageGuid ||
    raw.thumbnail?.imageGuid;

  if (directImageGuid && typeof directImageGuid === 'string' && directImageGuid.trim()) {
    const cleanGuid = directImageGuid.trim();
    const cdnUrl = cleanGuid.startsWith('http')
      ? cleanGuid
      : `https://playable-thumbnails.minecraft.net/images/${cleanGuid}`;
    if (!candidateUrls.includes(cdnUrl)) {
      candidateUrls.push(cdnUrl);
    }
  }
...
export function normalizeProduct(raw: any): ProductItem {
  ...
  return {
    ...
    title: String(raw.title || raw.name || raw.displayName || 'Bedrock Pack'),
    description: String(raw.description || raw.summary || raw.shortDescription || 'Дополнение для Minecraft Bedrock Edition.'),
    shortDescription: String(raw.shortDescription || raw.description || 'Minecraft Bedrock content.').slice(0, 150),
    type,
    category: String(raw.category || (type === 'world' ? 'Worlds' : type === 'skinpack' ? 'Skin Packs' : type === 'addon' ? 'Add-Ons' : type === 'resourcepack' ? 'Textures' : 'Bedrock')),
    edition: 'bedrock',
    creator,
    price,
    salePrice,
