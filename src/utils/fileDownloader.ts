...
export async function resolveRestApiFile(product: ProductItem): Promise<ResolvedRestApiFile> {
  ...
  const cleanTitle = (product.title || 'minecraft_item')
    ...
    .replace(/__+/g, '_');
  const defaultFilename = `${cleanTitle}.${fallbackExt}`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.url) {
        const fn = data.filename || defaultFilename;
        return {
          url: data.url,
          filename: fn,
          size: data.size,
    ...
    try {
      ...
      if (mrRes.ok) {
        ...
        if (Array.isArray(versions) && versions.length > 0) {
          ...
          if (primaryFile?.url) {
            const fn = primaryFile.filename || `${slugOrId}.jar`;
            return {
              url: primaryFile.url,
              filename: fn,
  ...
  if (cleanId.startsWith('cf-') || product.downloadUrl?.includes('curseforge.com')) {
    ...
    try {
      ...
      if (cfRes.ok) {
        ...
        if (directUrl) {
          const fn = latestFile?.name || `${slugOrId}.jar`;
          return {
            url: directUrl,
            filename: fn,
  ...
  return {
    url: `/api/download?url=${encodeURIComponent(product.downloadUrl || '')}&filename=${encodeURIComponent(defaultFilename)}&edition=${encodeURIComponent(edition)}`,
