...
// Ensure user profile in Firestore has default role: "user"
async function ensureFirestoreUserDoc(user: any, authProvider: 'google' | 'yandex', customData?: any) {
  try {
    const userRef = doc(db, 'users', user.uid);
    const docSnap = await getDoc(userRef);

    const email = customData?.email || user.email || '';
    const displayName = customData?.displayName || user.displayName || (authProvider === 'yandex' ? 'Пользователь Яндекс' : 'Пользователь Google');
    const nameParts = displayName.trim().split(' ');
    const firstName = customData?.firstName || nameParts[0] || 'Пользователь';
    const lastName = customData?.lastName || nameParts.slice(1).join(' ') || '';
    const photoURL = customData?.photoURL || user.photoURL || null;
    ...
    if (!docSnap.exists()) {
      ...
      if (firstName && firstName !== 'Гость') updatePayload.firstName = firstName;
...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  const [loadingType, setLoadingType] = useState<'google' | 'yandex' | 'guest' | null>(null);
  const [error, setError] = useState<string | null>(null);

  if (!isOpen) return null;

  // Strict Google Authentication Handler (using strictly new GoogleAuthProvider())
  const handleGoogleLogin = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      if (result?.user) {
        await ensureFirestoreUserDoc(result.user, 'google');
      }
  ...
  // Strict Yandex ID Authentication Handler (using strictly new OAuthProvider('oidc.yandex'))
  const handleYandexLogin = async (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    soundManager.playClick();
    setError(null);
    setLoadingType('yandex');
    try {
      ...
      if (result?.user) {
        ...
        await ensureFirestoreUserDoc(result.user, 'yandex', extracted);
      }
      soundManager.playLevelUp();
      onClose();
    } catch (err: any) {
      ...
      if (err?.code !== 'auth/popup-closed-by-user') {
        setError(err?.message || 'Не удалось выполнить вход через Яндекс ID. Попробуйте еще раз.');
      }
    } finally {
      setLoadingType(null);
    }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-xs">
      ...
      >
        ...
        <div className="space-y-3.5 mb-6">
          ...
          {/* Yandex ID Sign In Option */}
          <button
            id="login-yandex-btn"
            onClick={handleYandexLogin}
            disabled={loadingType !== null}
            className="w-full bg-[#2a2a2d] hover:bg-[#34353a] text-white p-3.5 border-2 border-[#555] hover:border-[#fc3f1d] flex items-center justify-between transition-all group active:scale-[0.99] disabled:opacity-50"
          >
            <div className="flex items-center gap-3">
              {/* Yandex ID Badge */}
              <div className="w-7 h-7 bg-[#fc3f1d] rounded-full flex items-center justify-center shadow-sm shrink-0">
                <span className="text-white font-black text-sm leading-none select-none font-sans">
                  Я
                </span>
              </div>
              <div className="text-left">
                <div className="text-sm font-bold text-white group-hover:text-[#ff9482] transition-colors">
                  Войти через Яндекс ID
                </div>
                <div className="text-[11px] text-[#8e8e93]">
                  Быстрая и безопасная авторизация через Яндекс
                </div>
              </div>
            </div>
            {loadingType === 'yandex' ? (
              <Loader2 className="w-5 h-5 text-[#fc3f1d] animate-spin" />
            ) : (
              <span className="text-xs bg-[#fc3f1d] text-white px-2.5 py-1 font-bold">
                Выбрать
              </span>
            )}
          </button>
