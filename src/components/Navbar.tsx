      ...
      if (term.length > 0) {
        ...
        try {
          const [mrRes, cfRes] = await Promise.all([
            modrinthApi.searchMods({ query: term, limit: 5 }).catch(() => ({ products: [] })),
            cfwidgetApi.searchMods({ query: term, limit: 5 }).catch(() => ({ products: [] })),
          ]);

          const cfItems: EnhancedSuggestion[] = (cfRes.products || []).map((p) => ({
            id: p.id,
            title: p.title,
            type: 'Java / Bedrock дополнение',
          ...
          const allSuggestions = [...mrItems, ...cfItems].slice(0, 10);
