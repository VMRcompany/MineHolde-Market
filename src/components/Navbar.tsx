...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Debounced auto-suggest across all catalog items
  useEffect(() => {
      ...
      if (term.length > 0) {
        ...
        try {
          const mrRes = await modrinthApi
            .searchMods({ query: term, limit: 10, sort: 'newest' })
            .catch(() => ({ products: [] }));

          const mrItems: EnhancedSuggestion[] = (mrRes.products || []).map((p) => ({
            id: p.id,
            title: p.title,
            type: `${p.category || 'Java Edition'}`,
          ...
          setSuggestions(mrItems);
          setShowDropdown(mrItems.length > 0);
        } catch {
          setSuggestions([]);
        } finally {
  ...
  const navCategories: { label: string; value: string; icon: React.ComponentType<{ className?: string }>; badge?: string }[] = [
    { label: 'Моды', value: 'Mods', icon: Package },
    { label: 'Наборы ресурсов', value: 'Resource Packs', icon: Layers },
    { label: 'Наборы данных', value: 'Data Packs', icon: FileCode },
    { label: 'Шейдеры', value: 'Shaders', icon: Sparkles },
    { label: 'Модпаки', value: 'Modpacks', icon: Box },
    { label: 'Плагины', value: 'Plugins', icon: Puzzle },
    { label: 'Серверы', value: 'Servers', icon: Server },
  ];
              ...
              <div className="divide-y divide-[#262628]">
                {suggestions.map((item) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold text-[#82d458] tracking-wide uppercase">Бесплатно</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            >
              {(profile?.photoURL || user?.photoURL) ? (
                <img
                  ...
                  alt={profile?.displayName || user?.displayName || 'Профиль'}
