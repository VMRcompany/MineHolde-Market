...
export const VersionSelectModal: React.FC<VersionSelectModalProps> = ({
    ...
    async function loadVersions() {
      try {
        ...
        if (rawVersions.length > 0) {
          ...
          // If Modrinth has no versions directly, build authentic Java Edition version items
          const isMod = product.category === 'Mods' || product.type === 'mod';
          const ext = isMod ? 'jar' : 'zip';
          const cleanTitle = product.title.replace(/[^a-zA-Z0-9_-]/g, '_');
          const fallbackVersions: ModrinthFileVersion[] = [
            {
              id: 'v-latest',
              name: `${product.title} (Java 1.21.x)`,
              version_number: '1.21.4',
              game_versions: ['1.21.4', '1.21.1', '1.21'],
              version_type: 'release',
              loaders: isMod ? ['fabric', 'forge', 'neoforge'] : ['vanilla'],
              files: [
                {
                  url: `/api/download?id=${encodeURIComponent(product.id)}`,
                  filename: `${cleanTitle}-1.21.4.${ext}`,
                  primary: true,
                  size: 15000000,
                },
              ],
            ...
            {
              id: 'v-stable',
              name: `${product.title} (Java 1.20.1)`,
              version_number: '1.20.1',
              game_versions: ['1.20.1'],
              version_type: 'release',
              loaders: isMod ? ['fabric', 'forge'] : ['vanilla'],
              files: [
                {
                  url: `/api/download?id=${encodeURIComponent(product.id)}`,
                  filename: `${cleanTitle}-1.20.1.${ext}`,
                  primary: true,
                  size: 14500000,
                },
              ],
  ...
  const handleDownloadVersion = async (version: ModrinthFileVersion) => {
    ...
    const edition = 'java';
    let fileName = targetFile.filename || `${product.title.replace(/[^a-zA-Z0-9_-]/g, '_')}.jar`;
      ...
      >
        ...
        <div className="p-4 overflow-y-auto flex-1 space-y-2.5 bg-[#161618]">
          {isLoading ? (
            <div className="py-16 flex flex-col items-center justify-center gap-3 text-[#a0a0a5]">
              ...
              <p className="text-sm font-medium">Загрузка доступных версий...</p>
            </div>
          ) : error ? (
        ...
        <div className="px-4 py-2.5 bg-[#141416] border-t border-[#2b2b2e] flex items-center justify-between text-xs text-[#777]">
          <div className="flex items-center gap-1.5 text-[11px]">
            <Sparkles className="w-3.5 h-3.5 text-[#82d458]" />
            <span className="text-[#a0a0a5]">Оригинальные пакеты Java Edition</span>
          </div>
