...
export const ProductModal: React.FC<ProductModalProps> = ({
  ...
  // 1. First image is always the official cover splash (Заставка данного пака)
  // 2. Subsequent images are the official in-game screenshots strictly in sequence
  const coverUrl = product.bannerUrl || product.thumbnailUrl;
  const rawScreenshots = Array.isArray(product.screenshots) ? product.screenshots : [];
  const uniqueScreenshots = rawScreenshots.filter((s) => s && s !== coverUrl);
  ...
  const currentMedia = galleryItems[activeImageIndex] || galleryItems[0];
            ...
            <div className="lg:col-span-7 space-y-3">
              ...
              <div className="aspect-video w-full mc-panel-dark overflow-hidden relative group bg-[#111112] border-2 border-[#333]">
                <img
                  ...
                  alt={`${product.title} - ${currentMedia.label}`}
                  className="w-full h-full object-cover select-none transition-all duration-200"
                />

                {/* Cover vs Screenshot Type Badge */}
                <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5 items-start">
                  {currentMedia.type === 'cover' ? (
                    <div className="bg-[#1f2022]/95 border border-[#82d458] text-[#a4f576] text-[11px] font-bold px-2.5 py-1 shadow-lg flex items-center gap-1.5 backdrop-blur-xs">
                      <Sparkles className="w-3.5 h-3.5 text-[#ffd83d]" />
                      <span>1. ЗАСТАВКА ПАКА</span>
                    </div>
                  ) : (
                    <div className="bg-[#1f2022]/95 border border-[#555] text-white text-[11px] font-bold px-2.5 py-1 shadow-lg flex items-center gap-1.5 backdrop-blur-xs">
                      <Camera className="w-3.5 h-3.5 text-[#82d458]" />
                      <span>{currentMedia.badge.toUpperCase()}</span>
                    </div>
                  )}

                  {product.isSale && product.discountPercent && (
                    <div className="bg-[#d93829] border border-[#ff6b6b] text-white text-[10px] font-bold px-2 py-0.5 shadow-lg uppercase tracking-wider">
                ...
                {/* Bottom Media Label strip */}
                <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/90 via-black/50 to-transparent p-2.5 pt-6 flex items-center justify-between text-xs text-white">
                  <div className="flex items-center gap-1.5">
                    {currentMedia.type === 'cover' ? (
                      <span className="text-[#a4f576] font-bold">⭐ Официальная заставка пака (Key Art)</span>
                    ) : (
                      <span className="text-[#ddd] font-medium">📷 Игровой скриншот #{activeImageIndex}</span>
                    )}
                  </div>
                  <span className="text-[11px] text-[#8e8e93]">Используйте стрелки ← / → для переключения</span>
                </div>
              </div>

              {/* Thumbnails list strictly ordered */}
              {galleryItems.length > 1 && (
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-[11px] text-[#8e8e93] px-0.5">
                    <span>Галерея из Minecraft Marketplace API:</span>
                    <span>1. Заставка → 2. Скриншоты по порядку</span>
                  </div>
                    ...
                    {galleryItems.map((item, idx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`absolute bottom-0 inset-x-0 text-[9px] font-bold text-center py-0.5 uppercase truncate ${
                              item.type === 'cover'
                                ? 'bg-[#235317]/90 text-[#a4f576]'
                                : 'bg-[#121213]/90 text-[#ddd]'
                            }`}
                          >
                            {item.type === 'cover' ? 'Заставка' : `Скрин ${idx}`}
                          </div>
                        </button>
                      );
                    })}
