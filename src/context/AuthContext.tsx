...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Strict Google Login (Strictly uses new GoogleAuthProvider())
  const loginWithGoogle = async () => {
    try {
      ...
      if (result?.user) {
        ...
        const isUserAdmin = isAdminUser(result.user.email);
        const isUserVip = isVipUser(result.user.email);
        const { firstName, lastName } = parseNames(result.user.displayName, result.user.email);
        ...
        if (!docSnap.exists()) {
          await setDoc(
            userRef,
            {
              ...
              displayName: result.user.displayName || 'Пользователь Google',
              firstName,
              lastName,
  ...
  // Strict Yandex ID Login (Strictly uses new OAuthProvider('oidc.yandex'))
