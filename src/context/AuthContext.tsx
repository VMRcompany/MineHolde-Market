        ...
        unsubscribeDoc = onSnapshot(userRef, async (docSnap) => {
          if (docSnap.exists()) {
            ...
            setProfile({
              ...
              downloadedPacks: Array.isArray(data.downloadedPacks) ? data.downloadedPacks : [],
              isAnonymous: currentUser.isAnonymous,
              createdAt: data.createdAt,
              lastLoginAt: data.lastLoginAt,
            });
          } else {
            // New user initial state - coins default to 0 for all users as instructed
            const { firstName, lastName } = parseNames(currentUser.displayName, currentUser.email);
            const initialData: Record<string, any> = {
              ...
              coins: 0, // default 0 MineHolde coins
            ...
            setProfile({
              ...
              downloadedPacks: initialData.downloadedPacks,
            } as UserProfileData);
          }
          setLoading(false);
        }, (error) => {
          console.warn('Firestore subscription error (fallback to local state):', error);
          setLoading(false);
        });
      } else {
        setProfile(null);
