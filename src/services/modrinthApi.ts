...
export function normalizeModrinthProject(hit: any): ProductItem {
  ...
  if (projType === 'resourcepack') {
    ...
    categoryName = 'Наборы ресурсов';
  } else if (projType === 'datapack') {
    itemType = 'addon';
    categoryName = 'Наборы данных';
  } else if (projType === 'shader') {
    itemType = 'resourcepack';
    categoryName = 'Шейдеры';
  } else if (projType === 'modpack') {
    itemType = 'mashup';
    categoryName = 'Модпаки';
  } else if (projType === 'plugin') {
    itemType = 'addon';
    categoryName = 'Плагины';
  } else if (projType === 'server' || hit.is_server) {
    itemType = 'world';
...
class ModrinthApiService {
  ...
  async searchMods(options: ModrinthSearchOptions = {}): Promise<{
    ...
    // Filter by category facets if provided or default to mod when no query
    if (facets !== undefined) {
      if (facets) queryParams.set('facets', facets);
    } else if (projectType) {
