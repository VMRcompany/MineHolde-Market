...
export default function App() {
  ...
  const handleOpenProduct = useCallback((product: ProductItem, updateUrl = true) => {
    productRegistryRef.current.set(product.id, product);
    if (product.uuid) productRegistryRef.current.set(product.uuid, product);
  ...
  // Handle Product Selection by unique ID (ensures 100% accurate product modal opening and routing)
  const handleSelectProductById = useCallback(
    async (id: string, initialProduct?: ProductItem, updateUrl = true) => {
      ...
      if (productRegistryRef.current.has(id)) {
        handleOpenProduct(productRegistryRef.current.get(id)!, updateUrl);
        return;
      }
      ...
      if (id.startsWith('cf-')) {
        try {
          ...
          if (exact) {
            handleOpenProduct(exact, updateUrl);
            return;
          }
        } catch (err) {
      ...
      try {
        ...
        if (details && details.id) {
          handleOpenProduct(details, updateUrl);
        }
      } catch {
        // Not found
      }
  ...
  // Initial URL check on mount (deep linking support)
  useEffect(() => {
