...
export default function App() {
  ...
  const handleCloseProductModal = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (currentPath.startsWith('/mods/') || currentPath.startsWith('/skins/')) {
        const fallbackPath = activeCategory === 'Servers' ? '/servers' : '/';
  ...
  const handleSelectProductById = useCallback(
    async (rawId: string | number, initialProduct?: ProductItem, updateUrl = true) => {
      ...
      const strippedId = lowerId.replace(/^(mr-|cf-|mod-)/, '');

      // 1. Check in global registry
      const reg = productRegistryRef.current;
      if (reg.has(id)) {
        handleOpenProduct(reg.get(id)!, updateUrl);
        return;
      }
      if (reg.has(lowerId)) {
        handleOpenProduct(reg.get(lowerId)!, updateUrl);
        return;
      }
      if (reg.has(strippedId)) {
        handleOpenProduct(reg.get(strippedId)!, updateUrl);
        return;
      }
      if (reg.has(`mr-${strippedId}`)) {
        handleOpenProduct(reg.get(`mr-${strippedId}`)!, updateUrl);
        return;
      }
      if (reg.has(`cf-${strippedId}`)) {
        handleOpenProduct(reg.get(`cf-${strippedId}`)!, updateUrl);
        return;
      }

      // 2. Check current in-memory products array
      const localFound = products.find((p) => {
        const pId = String(p.id).toLowerCase();
        const pUuid = String(p.uuid || '').toLowerCase();
        const pSlug = String((p as any).slug || '').toLowerCase();
        const pProjectId = String((p as any).project_id || '').toLowerCase();
        ...
        return (
          ...
          pUuid === strippedId ||
          pSlug === strippedId ||
          pProjectId === strippedId ||
          pId.replace(/^(mr-|cf-|mod-)/, '') === strippedId ||
          pUuid.replace(/^(mr-|cf-|mod-)/, '') === strippedId ||
          pTitleSlug === strippedId
        );
      });
      ...
      // 3. Direct Modrinth API lookup by ID or slug (Create, Sodium, Iris, JEI, etc.)
      try {
        const mrRes = await fetch(`https://api.modrinth.com/v2/project/${encodeURIComponent(strippedId)}`, {
          headers: {
        ...
        console.warn('Modrinth project lookup notice:', err);
      }

      // 4. Direct CFWidget API lookup by CurseForge ID or slug
      try {
        const cfRes = await fetch(`https://api.cfwidget.com/minecraft/mc-mods/${encodeURIComponent(strippedId)}`, {
          headers: {
            Accept: 'application/json',
            'User-Agent': 'MineHolde/1.0.0 (contact@mineholde.market)',
          },
        ...
        if (cfRes.ok) {
          ...
          if (cfData && (cfData.id || cfData.title || cfData.name)) {
            const normalized = normalizeCFWidgetProject(cfData);
            if (normalized) {
              registerProductInRegistry(normalized);
              handleOpenProduct(normalized, updateUrl);
              return;
        ...
        console.warn('CFWidget project lookup notice:', err);
      }

      // 5. Search fallback on Modrinth (STRICT matching, NEVER take products[0] blindly!)
      try {
        const mrSearch = await modrinthApi.searchMods({ query: strippedId, limit: 10 });
        if (mrSearch.products && mrSearch.products.length > 0) {
          const matched = mrSearch.products.find((p) => {
            const pId = String(p.id).toLowerCase();
            const pUuid = String(p.uuid || '').toLowerCase();
            const pTitle = p.title.toLowerCase();
            const pStripped = pId.replace(/^(mr-|cf-)/, '');
            const pUuidStripped = pUuid.replace(/^(mr-|cf-)/, '');
            return (
              pId === lowerId ||
              pUuid === lowerId ||
              pStripped === strippedId ||
              pUuidStripped === strippedId ||
              pTitle === strippedId ||
              pTitle.replace(/\s+/g, '-') === strippedId
            );
          });
          if (matched) {
        ...
        console.warn('Modrinth search fallback notice:', err);
      }

      // 6. Search fallback on CFWidget (STRICT matching, NEVER take products[0] blindly!)
      try {
        const cfSearch = await cfwidgetApi.searchMods({ query: strippedId, limit: 10 });
      ...
      // 7. If product physically does not exist -> Display 404 page!
      setIs404(true);
      setSelectedProduct(null);
    },
    [handleOpenProduct, products, registerProductInRegistry]
  );
  ...
  // Initial URL check on mount (deep linking & route handling)
  useEffect(() => {
    ...
    const match = path.match(/^\/(mods|skins)\/([^/]+)/);
    if (match) {
      const idFromUrl = decodeURIComponent(match[2]);
      setIs404(false);
      handleSelectProductById(idFromUrl, undefined, false);
      return;
    }
  ...
  }, [activeCategory, handleSelectProductById]);
  ...
  const handleSelectCategory = (cat: string) => {
    if (cat === 'Creators') {
      ...
      if (
        ...
        window.location.pathname.startsWith('/mods/') ||
        window.location.pathname.startsWith('/skins/')
      ) {
        window.history.pushState(null, '', '/');
      }
    }
    ...
    async (category: string, sort: string) => {
      ...
      try {
        ...
        const finalList = Array.from(uniqueMap.values()).filter(
          (p) => p.id.startsWith('mr-') || p.id.startsWith('cf-')
        );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        const searchList = Array.from(uniqueMap.values()).filter(
          (p) => p.id.startsWith('mr-') || p.id.startsWith('cf-')
        );
    ...
    try {
      ...
      if (newMods.length > 0) {
        ...
        setProducts((prev) => {
          ...
          const merged = Array.from(uniqueMap.values()).filter(
            (p) => p.id.startsWith('mr-') || p.id.startsWith('cf-')
          );
